...
#gets all attendance history of one student  with  lessons  check in check out  times ,lateness, early leaving, time in class  and status
async def get_student_attendance(student_id):
    conn=await get_connection()
    try:
        attendance=await conn.fetch("""
        select ar.id,ar.lesson_id,l.subject,l.lesson_date,
        l.start_time,l.end_time,ar.check_in_time,ar.check_out_time,
        ar.late_minutes,ar.early_leave_minutes,ar.time_in_class,ar.status from attendance_records ar
        ...
        await conn.close()
#gets all students who have an attendance record for a specific lesson with their name, check in check out , lateness, time in class and status
async def get_lesson_attendance(lesson_id):
    conn=await get_connection()
    try:
        attendance=await conn.fetch("""
        select ar.id,ar.student_id,u.full_name,ar.check_in_time,
        ar.check_out_time,ar.late_minutes,ar.early_leave_minutes,
        ar.time_in_class,ar.status from attendance_records ar
        ...
        await conn.close()
#the attendance for every student across every lesson belonging to one group
async def get_group_attendance(group_id):
    conn=await get_connection()
    try:
        attendance=await conn.fetch("""
        select ar.id,ar.lesson_id,ar.student_id,u.full_name,
        l.subject,l.lesson_date,ar.check_in_time,ar.check_out_time,
        ar.late_minutes,ar.early_leave_minutes,ar.time_in_class,ar.status from attendance_records ar
        join lessons l on ar.lesson_id=l.id
        join students s on ar.student_id=s.id
        join users u on s.user_id=u.id
...
async def get_attendance_percentage(student_id):
    ...
    try:
        ...
        select count(*) as total_lessons, count(*) filter (where status in ('present','late','left_early')) as attended_lessons from attendance_records
        where student_id=$1
        """,student_id)
        ...
        await conn.close()
#find the students check in time compare it with the lesson start time calculate the late minutes and save them
async def calculate_late_minutes(lesson_id,student_id):
    conn=await get_connection()
    try:
        result=await conn.fetchrow("""
        select ar.check_in_time,l.start_time from attendance_records ar
        join lessons l on ar.lesson_id=l.id
...
async def edit_attendance(attendance_id,edited_by,old_status,new_status,edit_reason):
    ...
    try:
        ...
        await conn.execute("""
        insert into attendance_edits(attendance_id,edited_by,old_status,new_status,edit_reason)
        values($1,$2,$3,$4,$5)
...
async def get_attendance_edits(attendance_id):
    ...
    try:
        ...
        select ae.id,ae.attendance_id,ae.edited_by,
        u.full_name as editor_name,ae.old_status,ae.new_status,
        ae.edit_reason,ae.edited_at from attendance_edits ae
